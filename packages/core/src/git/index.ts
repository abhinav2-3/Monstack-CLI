...
export const initializeGit = async (projectPath: string): Promise<void> => {
  ...
  try {
    ...
    await execa('git', ['commit', '-m', 'Initial commit from MonStack CLI'], { cwd: projectPath });
    console.log(chalk.green('✅ Git initialized successfully.'));
  } catch (error) {
    console.log(chalk.yellow('⚠️ Failed to initialize git repository. This is non-blocking.'));

...
async function test() {
  try {
    ...
    console.log(
      `[${validateMiddleware ? 'OK' : 'FAIL'}] validation middleware exists`,
    );

    // Check Swagger files
    const swaggerConfig = await fs.pathExists(
      path.join(targetDir, 'src/config/swagger.ts'),
    );
    ...
    const hasSwaggerImport = appContent.includes(
      "import swaggerUi from 'swagger-ui-express';",
    );
    const hasSwaggerMiddleware = appContent.includes(
      "app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerSpec));",
    );
    console.log(
      `[${hasSwaggerImport ? 'OK' : 'FAIL'}] swagger imports injected into app.ts`,
    );
    console.log(
      `[${hasSwaggerMiddleware ? 'OK' : 'FAIL'}] swagger middleware injected into app.ts`,
    );
    ...
    const swaggerExists =
      pkg.dependencies && pkg.dependencies['swagger-ui-express'];
    console.log(`[${zodExists ? 'OK' : 'FAIL'}] zod dependency injected`);
    console.log(
      `[${swaggerExists ? 'OK' : 'FAIL'}] swagger dependency injected`,
    );
  } catch (err) {
